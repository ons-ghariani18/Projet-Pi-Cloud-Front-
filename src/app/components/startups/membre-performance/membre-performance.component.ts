...
export class MembrePerformanceComponent implements OnChanges {
  ...
  loadPerformance(): void {
    ...
    this.performanceService.getPerformance(this.startupId).subscribe({
      ...
      error: () => {
        this.toastService.show('Erreur chargement performance membres');
        this.loading = false;
      }
    });
  }
  ...
  actionSuspendre(m: MembrePerformance): void {
    this.performanceService.suspendre(m.membreId).subscribe({
      next: () => {
        this.toastService.show(`Membre ${m.nomPrenom} suspendu`);
        this.loadPerformance();
      },
      error: () => this.toastService.show('Erreur suspension membre')
    });
  }
  ...
  actionRevoquer(m: MembrePerformance): void {
    this.performanceService.revoquer(m.membreId).subscribe({
      next: () => {
        this.toastService.show(`Accès révoqué pour ${m.nomPrenom}`);
        this.loadPerformance();
      },
      error: () => this.toastService.show('Erreur révocation membre')
    });
  }

  actionRelancer(m: MembrePerformance): void {
    this.performanceService.relancer(m.membreId).subscribe({
      next: () => this.toastService.show(`Relance envoyée à ${m.nomPrenom}`),
      error: () => this.toastService.show('Erreur relance membre')
    });
  }
