  ...
  setStatus(status: 'approved' | 'pending' | 'rejected'): void {
    if (this.startup && this.startup.id !== undefined) {
      this.startupService.updateStartupStatus(this.startup.id, status).subscribe({
        next: () => {
          const label = status === 'approved' ? 'Approuvée' : (status === 'pending' ? 'En attente' : 'Rejetée');
          this.toastService.show(`Statut mis à jour : ${label}`);
          this.close();
        },
        error: (err) => {
          console.error('Error updating status', err);
          this.toastService.show('Erreur lors de la mise à jour du statut');
        }
      });
    }
  }
