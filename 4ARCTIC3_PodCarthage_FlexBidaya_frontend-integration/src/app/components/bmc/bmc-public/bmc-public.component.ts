...
export class BmcPublicComponent implements OnInit, OnDestroy {
  ...
  // ✅ Mapper keys (block.id) -> French keys (backend)
  ...
  private normalizeBlockKey(key: string): string {
    return this.BLOCK_KEY_MAP[key] || key;
  }
  ...
  joinAndLoad() {
    ...
    this.bmcService.joinByToken(this.token!).subscribe({
      next: (info) => {
        ...
        this.wsService.subscribe(sid).subscribe((event: any) => {
  ...
  loadProposals() {
    this.bmcService.getProposalsByToken(this.token!).subscribe({
      ...
      error: (err: any) => console.error("Erreur lors du chargement des propositions.", err)
    });
  }
  ...
  submitProposal() {
    ...
    const frenchKey = this.normalizeBlockKey(this.currentBlock.id);
  ...
  getPendingForBlock(blockId: string): BmcProposal[] {
    const frenchKey = this.normalizeBlockKey(blockId);
    return this.proposals.filter(p => p.blockName === blockId || p.blockName === frenchKey);
  }

  onTyping(event: Event): void {
    const frenchKey = this.normalizeBlockKey(this.currentBlock.id);
    console.log('>>> MEMBRE envoie TYPING sur startupId:', this.membreInfo.startupId, 'blockName:', frenchKey);
