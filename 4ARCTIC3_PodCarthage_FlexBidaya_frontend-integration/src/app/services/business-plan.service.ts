...
export class BusinessPlanService {
  private apiUrl = `${environment.apiUrl}/bmc`;
  ...
  delete(id: number): Observable<any> {
    return this.http.delete(`${this.apiUrl.replace('/bmc', '/business-plans')}/${id}`);
  }
  ...
  getByStartupId(startupId: number): Observable<BusinessPlanDTO[]> {
    return this.http.get<BusinessPlanDTO[]>(`${environment.apiUrl}/business-plans/startup/${startupId}`);
  }
}
